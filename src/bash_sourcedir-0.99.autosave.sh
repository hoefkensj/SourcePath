...
""";
	...
	{ 
		...
		function _progress () { 
			#~	 G   m  m   STRING
			local toprint mustprint maxprint
			maxprint=50
			toprint=$1
			[[ -n "$5" ]] && mustprint="$5" && maxprint=$(( maxprint - ${#mustprint}  - 3 ))

			while true  ; do
				[[ ${#toprint} > $maxprint ]]  && toprint=".../${toprint#*/*/}" 
				[[ ${#toprint} < $((maxprint+1)) ]] && break ;
			done	
			toprint="${mustprint} ${toprint}"	
			_Gm  12  1  3   "$toprint  "
			_Gm "$2" 1 "$3" "$4" 
			_G 80
		};		
